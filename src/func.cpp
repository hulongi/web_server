...
int send_dir(bufferevent *bev, const char *dirname)
{
    ...
    for (i = 0; i < num; ++i)
    {
        ...
        else
        {
            ...
            {
                sprintf(buf + strlen(buf),
                        "<tr><td><a href=\"%s\">%s</a></td><td>%s</td><td>%ld</td><td><a href=\"%s\" download=%s>点击下载</a></td></tr>\n",
                        encoded_name, dirinfo[i]->d_name, timestr, sb.st_size, encoded_name, dirinfo[i]->d_name);
            }
        }
